 ...
 * (OpenRouter Coinbase Business Checkout). The operator Privy wallet funds this
 * one pinned address via a single constrained transfer
 * (OperatorWalletPort.withdrawToSteward); a human then completes the vendor
...
export const repoSpecSchema = z
	.object({
		...
		payments_out: z
			.object({
				/** Human-custodied wallet that settles vendor invoices (OpenRouter). */
